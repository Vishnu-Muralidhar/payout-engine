...
export default function Dashboard() {
  ...
  mutationFn: ({ amountPaise, idempotencyKey }: { amountPaise: number; idempotencyKey: string }) =>
    createPayout(amountPaise, testBankAccountId, idempotencyKey),
  ...
  onSuccess: () => {
    ...
    // ✅ generate new key ONLY after request lifecycle completes
    setIdempotencyKey(uuidv4());
  },

  onError: () => {
    // Optional:
    // You can keep the same key to allow safe retry
    // OR regenerate depending on UX choice
  },
  });
  ...
  const handlePayoutSubmit = (e: React.FormEvent) => {
    ...
    payoutMutation.mutate({
      amountPaise: amountRs * 100,
      idempotencyKey, // same key used for retry
    });
  };
